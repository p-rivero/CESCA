/*
  ...
  ** Modified for detection of key breaks on ascii characters by Pol R.
...
static uint16_t CharBuffer=0; // Changed from uint8_t to uint16_t
...
// The other keymaps have been removed in order to reduce footprint. Download the keymap you need from
// https://github.com/PaulStoffregen/PS2Keyboard/blob/master/PS2Keyboard.cpp
    ...
    while (1) {
        ...
        if (s == 0xF0) {
            ...
            if (state & BREAK) {
                ...
                if (s == 0x12) {
                    state &= ~(BREAK | MODIFIER | SHIFT_L);
                    continue;
                } else if (s == 0x59) {
                    state &= ~(BREAK | MODIFIER | SHIFT_R);
                    continue;
                } else if (s == 0x11 && (state & MODIFIER)) {
                    state &= ~(BREAK | MODIFIER | ALTGR);
                    continue;
                }
            ...
            // New detection of key breaks
            if (state & BREAK) {
                state &= ~(BREAK | MODIFIER);
                if (c) return c + PS2_KEY_BREAK;
            }
            if (c) return c;
        }
    }
...
void PS2Keyboard::clear() {
    CharBuffer = UTF8next = 0;
    
    // Also clear the scancode buffer
    head = tail = 0;
}
...
int PS2Keyboard::read() {
    ...
    if (result) {
        ...
        // New detection of key breaks
        key_break = result >= PS2_KEY_BREAK;
        result &= PS2_KEY_BREAK-1;
    ...
    if (!result) return -1;
    if (key_break) result |= PS2_KEY_BREAK;
    return result;
}
